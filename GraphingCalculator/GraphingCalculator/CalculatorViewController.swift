...
//  GraphingCalculator
//
//  Created by sukhjeet singh sandhu on 21/06/16.
...
class CalculatorViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: "graph", style: .plain, target: self, action: #selector(CalculatorViewController.loadGraphViewController))
        createDisplay()
    ...
    fileprivate func createDisplay() {
        ...
        view.addConstraint(NSLayoutConstraint(item: display, attribute: .top, relatedBy: .equal, toItem: view, attribute: .top, multiplier: 1.0, constant: UIApplication.shared.statusBarFrame.size.height + (navigationController?.navigationBar.frame.height)! + 4))
    ...
    //MARK: Segueing to next MVC
    func loadGraphViewController() {
        //It will only segue to the gvc if there is a variable present in equation or no equation is there.
        var isAnyVariable = false
        if let arrayOfOps = savedOperation as? [AnyObject] {
            for op in arrayOfOps {
                if let op = op as? String {
                    if op == "M" {
                        isAnyVariable = true
                    }
                }
